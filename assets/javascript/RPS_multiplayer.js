	...
	var RPS = {
		...
		connectFirebase: function() {
			...
			this.db.ref('/players').on("value", function(snapshot) {
				...
				if (thisVal && !RPS.disconnected) {
					...
					if (lastIndex === 0) {
						// this is player1  
						if (!RPS.playersLoaded && !RPS.roundWinner) {
			...
			this.db.ref('/turn').on("value", function(snapshot) {
				var thisValTurn = snapshot.val();
				// do nothing if no data exists (on load)
				if (thisValTurn) {
					//do this only if both players are here
					if ((RPS.player1_name.length > 0) && (RPS.player2_name.length > 0)) {
						RPS.yourTurn(thisValTurn);
					};
				};
		...
		creatUser: function(name) {
			...
			if (!RPS.player1_name) {
				...
				RPS.db.ref('/players').child('1').onDisconnect().remove(function() { 
				...
				RPS.db.ref('/players').child('2').onDisconnect().remove(function() { 
					/*if (RPS.playersLoaded) { 
		...
		playerDisconnected: function(player) {
			RPS.disconnected = true;
			var pName;

			if (player === 1) {
				pName = RPS.player1_name;
				$('#player1_main').find('.pName').text('Waiting for Player 1');
				$('#player1_main').find('.large, rpsBtnHolder').empty();
				$('#player2_main').find('.pResults').empty();
			} else {
				pName = RPS.player2_name;
				$('#player2_main').find('.pName').text('Waiting for Player 2');
				$('#player2_main').find('.large, rpsBtnHolder').empty();
				$('#player1_main').find('.pResults').empty();
			}
			
			RPS.db.ref('/chats').remove();
			RPS.db.ref('/turn').remove();

			$('#chatResults').html('<h4 class="disconnectMessage">' + pName + ' Has disconnected.</h4>');
